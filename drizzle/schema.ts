...
/**
 * Task edit proposals - non-assignees can propose edits to tasks
 */
export const taskEditProposals = mysqlTable("task_edit_proposals", {
  id: int("id").autoincrement().primaryKey(),
  taskId: int("taskId").notNull().references(() => tasks.id, { onDelete: "cascade" }),
  proposedBy: int("proposedBy").notNull().references(() => householdMembers.id),
  proposedChanges: json("proposedChanges").$type<Record<string, any>>().notNull(),
  status: mysqlEnum("status", ["pending", "approved", "rejected"]).default("pending").notNull(),
  reviewedBy: int("reviewedBy").references(() => householdMembers.id),
  reviewedAt: timestamp("reviewedAt"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});

export type TaskEditProposal = typeof taskEditProposals.$inferSelect;
export type InsertTaskEditProposal = typeof taskEditProposals.$inferInsert;

/**
 * Dependency proposals - proposed task dependencies awaiting approval
 */
export const dependencyProposals = mysqlTable("dependency_proposals", {
  id: int("id").autoincrement().primaryKey(),
  taskId: int("taskId").notNull().references(() => tasks.id, { onDelete: "cascade" }),
  dependsOnTaskId: int("dependsOnTaskId").notNull().references(() => tasks.id, { onDelete: "cascade" }),
  dependencyType: mysqlEnum("dependencyType", ["prerequisite", "followup"]).notNull(),
...
export type DependencyProposal = typeof dependencyProposals.$inferSelect;
export type InsertDependencyProposal = typeof dependencyProposals.$inferInsert;

/**
 * Notifications - system notifications for household members
 */
export const notifications = mysqlTable("notifications", {
  ...
  relatedItemType: mysqlEnum("relatedItemType", ["task", "project", "proposal", "shopping", "other"]),
  relatedItemId: int("relatedItemId"),
...
/**
 * Notification settings - per-member notification preferences
 */
export const notificationSettings = mysqlTable("notification_settings", {
  id: int("id").autoincrement().primaryKey(),
  memberId: int("memberId").notNull().references(() => householdMembers.id, { onDelete: "cascade" }).unique(),
  taskAssigned: boolean("taskAssigned").default(true).notNull(),
  taskCompleted: boolean("taskCompleted").default(true).notNull(),
  taskDueSoon: boolean("taskDueSoon").default(true).notNull(),
  taskOverdue: boolean("taskOverdue").default(true).notNull(),
  editProposal: boolean("editProposal").default(true).notNull(),
  dependencyProposal: boolean("dependencyProposal").default(true).notNull(),
  proposalApproved: boolean("proposalApproved").default(true).notNull(),
  proposalRejected: boolean("proposalRejected").default(true).notNull(),
  projectUpdate: boolean("projectUpdate").default(true).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});

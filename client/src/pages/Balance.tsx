...
import { ArrowLeft, Banknote, BriefcaseBusiness, ChevronLeft, History as HistoryIcon, Pencil, Plus, Trash2, Users } from "lucide-react";
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {entries.length === 0 ? <p className="text-sm text-muted-foreground py-4">{t("balance:noEntries")}</p> : entries.map((entry: any) => {
              ...
              return <div key={entry.id} className="flex gap-3 py-3 border-b last:border-0"><div className={`mt-0.5 h-9 w-9 rounded-full grid place-items-center ${entry.entryType === "payment" ? "bg-emerald-100 text-emerald-700" : "bg-sky-100 text-sky-700"}`}>{entry.entryType === "payment" ? <Banknote className="h-4 w-4" /> : <BriefcaseBusiness className="h-4 w-4" />}</div><div className="min-w-0 flex-1"><div className="flex flex-wrap justify-between gap-2"><p className="font-medium">{entry.memberName}</p><p className="font-semibold">{entry.entryType === "payment" ? Number(entry.amount).toLocaleString(i18n.language, { style: "currency", currency: "EUR" }) : minutesLabel(Number(entry.minutes))}</p></div><p className="text-sm mt-0.5">{entry.description}</p><p className="text-xs text-muted-foreground mt-1">{format(new Date(entry.occurredAt), "d. MMMM yyyy", { locale: dateFnsLocale })}{entry.sourceType !== "manual" ? ` · ${t(`balance:${entry.sourceType}`)}` : ""}</p></div><div className="flex flex-col gap-1">{entry.historyActivityId && <Button variant="ghost" size="icon" onClick={() => setLocation(`/history?activityId=${entry.historyActivityId}`)} aria-label={t("balance:viewHistory")}><HistoryIcon className="h-4 w-4" /></Button>}{canEdit && <><Button variant="ghost" size="icon" onClick={() => openEdit(entry)} aria-label={t("balance:edit")}><Pencil className="h-4 w-4" /></Button><Button variant="ghost" size="icon" className="text-destructive hover:text-destructive" onClick={() => window.confirm(t("balance:deleteConfirm")) && deleteMutation.mutate({ householdId, entryId: entry.id, editorMemberId: memberId })} aria-label={t("common:delete")}><Trash2 className="h-4 w-4" /></Button></>}</div></div>;
            })}
          </CardContent>
        </Card>
      </div>

...
import { Home, Plus, LogIn, Users, User, Settings, Backpack, Share2, Trash2, Copy, Check, Link2, X, ChevronLeft } from "lucide-react";
...
import { PlansackEditor } from "@/components/PlansackEditor";
...
export default function HouseholdSelection() {
  ...
  // Tab-State
  const [activeTab, setActiveTab] = useState<"households" | "plansack">("households");
  ...
  // Plansack State
  const [editingBagItemId, setEditingBagItemId] = useState<number | null>(null);
  ...
  const createHouseholdMutation = trpc.householdManagement.createHousehold.useMutation({
    ...
    onError: (error: any) => toast.error(error.message || t("householdSelection.createError", "Fehler beim Erstellen")),
  });

  const joinHouseholdMutation = trpc.householdManagement.joinHousehold.useMutation({
    onSuccess: (data: any) => {
      toast.success(t("householdSelection.joinSuccess", "Beigetreten: {{name}}", { name: data.household.name }));
      setJoinDialogOpen(false);
      setInviteCode("");
      refetchHouseholds();
    },
    onError: (error: any) => toast.error(error.message || t("householdSelection.joinError", "Fehler beim Beitreten")),
  });
  ...
  const switchHouseholdMutation = trpc.householdManagement.switchHousehold.useMutation({
    onSuccess: (data: any) => {
      ...
      toast.success(t("householdSelection.switchSuccess", "Willkommen in {{name}}!", { name: data.householdName }));
      setLocation("/");
    },
    onError: (error: any) => toast.error(error.message || t("householdSelection.switchError", "Fehler beim Wechseln")),
  });
  ...
  const deleteShareMutation = trpc.planBag.deleteShareLink.useMutation({
    onSuccess: () => { utils.planBag.listBag.invalidate(); },
    onError: () => toast.error(t("common:error", "Fehler")),
  });
  ...
  const handleCreateHousehold = () => {
    if (!newHouseholdName.trim()) { toast.error(t("householdSelection.enterHouseholdName", "Name eingeben")); return; }
    createHouseholdMutation.mutate({ householdName: newHouseholdName.trim(), language: newHouseholdLanguage } as any);
  };

  const handleJoinHousehold = () => {
    if (!inviteCode.trim()) { toast.error(t("householdSelection.enterInviteCode", "Code eingeben")); return; }
  ...
  const handleLogout = () => {
    ...
    toast.success(t("auth:logout.success", "Abgemeldet"));
    setLocation("/login");
  };

  if (!token) { setLocation("/login"); return null; }
  if (userLoading) return <div className="min-h-screen flex items-center justify-center"><p>{t("common:common.messages.loading", "Laden...")}</p></div>;
  if (!currentUser) return <div className="min-h-screen flex items-center justify-center"><p>{t("householdSelection.userLoadError", "Fehler beim Laden...")}</p></div>;

  // Wenn ein Plansack-Eintrag bearbeitet wird → Editor-Ansicht
  const editingItem = editingBagItemId !== null ? (bagItems as any[]).find((i: any) => i.id === editingBagItemId) : null;
  if (editingItem) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
        <div className="max-w-2xl mx-auto">
          <div className="flex items-center gap-3 mb-4">
            <Button variant="outline" size="sm" onClick={() => setEditingBagItemId(null)}>
              <ChevronLeft className="w-4 h-4 mr-1" />
              {t("common:back", "Zurück")}
            </Button>
            <div className="flex items-center gap-2">
              <Backpack className="w-5 h-5 text-violet-600" />
              <h2 className="font-semibold">{editingItem.snapshot?.name ?? "Vorlage bearbeiten"}</h2>
            </div>
          </div>
          <PlansackEditor
            bagItemId={editingItem.id}
            initialSnapshot={editingItem.snapshot as PlanBagSnapshot}
            onSaved={() => utils.planBag.listBag.invalidate()}
          />
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
      {/* Sprach-Buttons */}
      <div className="fixed top-4 right-4 flex items-center gap-1 flex-wrap max-w-48 justify-end">
      ...
      <Card className="w-full max-w-2xl">
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <CardDescription className="mt-2">
                {t("householdSelection.subtitle", "Willkommen, {{name}}!", { name: currentUser.name })}
              </CardDescription>
            </div>
        ...
        <CardContent className="space-y-4">
          ...
          {/* ─── Tabs: Haushalte / Plansack ───────────────────────── */}
          <div className="flex gap-1 bg-muted rounded-lg p-1">
            <button
              className={`flex-1 py-2 px-3 rounded-md text-sm font-medium transition-colors flex items-center justify-center gap-2 ${
                activeTab === "households" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
              }`}
              onClick={() => setActiveTab("households")}
            >
              <Users className="w-4 h-4" />
              {t("householdSelection.yourHouseholds", "Haushalte")}
            </button>
            <button
              className={`flex-1 py-2 px-3 rounded-md text-sm font-medium transition-colors flex items-center justify-center gap-2 ${
                activeTab === "plansack" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
              }`}
              onClick={() => setActiveTab("plansack")}
            >
              <Backpack className="w-4 h-4" />
              {t("plankiste:plansack.title", "Plansack")}
              {(bagItems as any[]).length > 0 && (
                <span className="bg-violet-100 text-violet-700 text-xs font-bold rounded-full px-1.5 py-0.5 leading-none">
                ...
                <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
                  ...
                  <DialogContent className="max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                      ...
                      <DialogDescription>{t("householdSelection.createDescription", "Geben Sie einen Namen ein.")}</DialogDescription>
                    </DialogHeader>
                ...
                <Dialog open={joinDialogOpen} onOpenChange={setJoinDialogOpen}>
                  ...
                  <DialogContent>
                    <DialogHeader>
                      ...
                      <DialogDescription>{t("householdSelection.joinDescription", "Einladungscode eingeben.")}</DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                      <div className="space-y-2">
              ...
              {!households || households.length === 0 ? (
                <Card className="border-dashed">
                  <CardContent className="flex flex-col items-center justify-center py-8 text-center">
                    ...
                    <p className="text-sm text-gray-600">{t("householdSelection.noHouseholds", "Noch kein Haushalt.")}</p>
                  </CardContent>
                </Card>
              ) : (
                <div className="grid grid-cols-1 gap-3">
                  {households.map((household: any) => (
                    <Card key={household.householdId} className="cursor-pointer hover:shadow-md transition-shadow"
                      onClick={() => switchHouseholdMutation.mutate({ householdId: household.householdId })}>
                      <CardContent className="flex items-center justify-between p-4">
                        <div>
                          <h3 className="font-semibold text-lg">{household.householdName}</h3>
                          <p className="text-sm text-gray-600">{t("householdSelection.memberAs", "Als")}: {household.memberName}</p>
                        </div>
            ...
            <div className="space-y-3">
              {(bagItems as any[]).length === 0 ? (
                <Card className="border-dashed border-violet-200">
                  <CardContent className="flex flex-col items-center justify-center py-8 text-center">
                    <Backpack className="h-10 w-10 text-violet-300 mb-3" />
                    <p className="text-sm text-gray-500">{t("plankiste:plansack.empty", "Plansack ist leer")}</p>
                          ...
                          <div className="flex items-start gap-2">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex gap-3 mt-0.5 text-xs text-muted-foreground">
                                {shoppingCount > 0 && <span>🛒 {shoppingCount}</span>}
                            ...
                            <div className="flex flex-col gap-1 flex-shrink-0">
                              <Button size="sm" variant="outline" className="h-7 px-2 text-xs border-violet-200 text-violet-700"
                                onClick={() => setEditingBagItemId(item.id)}>
      ...
      <Dialog open={importBagId !== null} onOpenChange={open => { if (!open) { setImportBagId(null); setImportHouseholdId(""); } }}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{t("plankiste:plansack.importDesc", "Vorlage wird als neue Vorlage in den gewählten Haushalt importiert.")}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              ...
              {households && (households as any[]).length > 0 ? (
                ...
                <p className="text-sm text-muted-foreground">{t("householdSelection.noHouseholds", "Keine Haushalte")}</p>
              )}
            </div>
      ...
      <AlertDialog open={deleteBagId !== null} onOpenChange={open => { if (!open) setDeleteBagId(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t("plankiste:plansack.removeDesc", "Der Eintrag wird gelöscht.")}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <Dialog open={shareBagItem !== null} onOpenChange={open => { if (!open) { setShareBagItem(null); setShareToken(null); } }}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{shareBagItem && `"${shareBagItem.name}"`}</DialogDescription>
          </DialogHeader>
          {shareToken ? (
            <div className="space-y-3">
            ...
            <p className="text-sm text-muted-foreground">{t("plankiste:plansack.shareCreateHint", "Link erstellen zum Teilen.")}</p>
          )}

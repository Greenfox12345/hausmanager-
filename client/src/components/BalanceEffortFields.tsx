  ...
  return (
    <div className="rounded-lg border bg-muted/30 p-3 space-y-3">
      ...
      {rows.map((row, index) => <div key={row.id} className="rounded-md border bg-background p-3 space-y-2">
        <div className="flex items-center justify-between gap-2"><p className="text-xs font-medium">{t("balance:effortNumber", { number: index + 1 })}</p><Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => setRows((current) => current.filter((candidate) => candidate.id !== row.id))} aria-label={t("balance:removeEffort")}><Trash2 className="h-4 w-4" /></Button></div>
        <div className="grid grid-cols-2 gap-2">
